...
df <- read_csv("E:/DATA_SCIENCE/Projects/weeks0-4/qPCR_gene_expression.csv")
...
write_csv(df, "E:/DATA_SCIENCE/Projects/weeks0-4/w4_PCR_results_R.csv")
...
ggplot(df, aes(x = gene, y = fold_change, fill = regulation_r)) +   # fill=reg_r → color by regulation
  ...
  geom_errorbar(aes(ymin = fold_change - sd_fc, ymax = fold_change + sd_fc),
                width = 0.15) +
  geom_hline(yintercept = 1, linetype = "dashed",       # geom_hline() → add dashed reference line at y = 1
             color = "gray40") +
  scale_fill_manual(values = c("Upregulated" = "#d73027",              # scale_fill_manual() → custom color calls
...
ggsave("E:/DATA_SCIENCE/Projects/weeks0-4/w4_gene_foldchange_plot_r.png",   # ggsave() → exports last plot
...
# save summarized table as .csv and plot as .pdf file
write_csv(df_sum, "E:/DATA_SCIENCE/Projects/weeks0-4/w4_PCR_summary_clean_r.csv")        # write_csv() → save .csv
ggsave("E:/DATA_SCIENCE/Projects/weeks0-4/w4_PCR_plot_test_r.pdf", width = 6, height = 4)    # ggsave() → export plot

# uses kable() command from knitr library to make a nicer table format in the console output
df_sum %>%
  arrange(desc(mean_fc)) %>%                             # arrange() → sort by mean_fc descending
  select(gene, mean_fc, sd_fc, sem_fc, n) %>%
  knitr::kable(digits = 2, caption = "Fold Change Summary")  # kable() → neat Markdown table

#assign the df plot to p using ggplot, and apply the steelblue theme color
p <- ggplot(df, aes(x = gene, y = fold_change)) +
  geom_col(fill = "steelblue") +
  theme_minimal()

# uses ggpubr to call/display a plot of df.fold_change--but now processed by ggplot and ggpubr formating options
p + theme_pubr(base_size = 14) + 
  font("x", size = 16, face = "bold") +
  font("y", size = 16, face = "bold")                                         # theme_pubr() → journal-style theme
